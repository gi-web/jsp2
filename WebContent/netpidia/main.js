...
function checkId() {
    ...
    if(id.value === "") {
        ...
        error[0].innerHTML = "3~20자의 영문 소문자, 숫자와 특수기호(_),(-)만 사용 가능합니다.";
        error[0].style.display = "block";
    } else {
...
function checkPw() {
    ...
    if(pw1.value === "") {
        ...
        error[1].innerHTML = "4~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요.";
...
function isBirthCompleted() {
    ...
    if(!yearPattern.test(yy.value)) {
        error[4].innerHTML = "생년월일 6자리를 정확하게 입력하세요. ex)920122";
...
function checkAge() {
    if(Number(yy.value) > 210000) {       
        error[4].innerHTML = "미래에서 오셨군요. ^^";
        error[4].style.display = "block";
    } else if(Number(yy.value) > 060000) {
